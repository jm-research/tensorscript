#ifndef TENSORSCRIPT_IR_INSTRUCTIONS_H
#define TENSORSCRIPT_IR_INSTRUCTIONS_H

#include <map>
#include <vector>

#include "tensorscript/ir/constant.h"
#include "tensorscript/ir/enums.h"
...
class unmasked_load_inst : public load_inst {
 ...
 public:
  static unmasked_load_inst* create(value* ptr, const std::string& name = "",
                                    instruction* next = nullptr);
...
class reshape_inst : public retile_inst {
 ...
 public:
  static instruction* create(value* arg,
                             const type::tile_shapes_t& shape_suffix,
                             const std::string& name = "",
                             instruction* next = nullptr);
...
class get_num_program_inst : public builtin_inst {
 private:
  get_num_program_inst(type* ty, unsigned axis, const std::string& name,
                       instruction* next);
  std::string repr_impl() const {
    return "get_num_program(" + std::to_string(axis_) + ")";
  }
...
class atomic_exch_inst : public builtin_inst {
 private:
  atomic_exch_inst(value* ptr, value* val, const std::string& name = "",
                   instruction* next = nullptr);
 ...
 public:
  static instruction* create(value* ptr, value* val,
                             const std::string& name = "",
                             instruction* next = nullptr);
};
...
class atomic_add_inst : public builtin_inst {
 private:
  atomic_add_inst(value* ptr, value* val, const std::string& name = "",
                  instruction* next = nullptr);
...
class dot_inst : public builtin_inst {
 ...
 private:
  dot_inst(value* A, value* B, value* C, TransT AT, TransT BT,
           const std::string& name, instruction* next);
 ...
 public:
  static instruction* create(value* A, value* B, value* C, bool AT, bool BT,
                             const std::string& name = "",
                             instruction* next = nullptr);
  static instruction* create_nn(value* A, value* B, value* C,
                                const std::string& name = "",
                                instruction* next = nullptr);
  static instruction* create_nt(value* A, value* B, value* C,
                                const std::string& name = "",
                                instruction* next = nullptr);
  static instruction* create_tn(value* A, value* B, value* C,
                                const std::string& name = "",
                                instruction* next = nullptr);
  static instruction* create_tt(value* A, value* B, value* C,
                                const std::string& name = "",
                                instruction* next = nullptr);
...
class reduce_inst : public builtin_inst {
 public:
  enum op_t { ADD, SUB, MAX, MIN, FADD, FSUB, FMAX, FMIN };
...
class recoalesce_inst : public unary_inst {
 ...
 public:
  static recoalesce_inst* create(value* arg, const std::string& name = "",
                                 instruction* next = nullptr);
...
class make_range_dyn : public instruction {
 ...
 public:
  static make_range_dyn* create(type* ty, const std::string& name = "",
                                instruction* next = nullptr);
};
...
#endif  // TENSORSCRIPT_IR_INSTRUCTIONS_H