#ifndef TENSORSCRIPT_TRITON_IR_MODULE_H
#define TENSORSCRIPT_TRITON_IR_MODULE_H
...
class module {
 ...
 public:
  ...
  // Register global
  void register_global(const std::string& name, ir::value* x) {
    globals_[name] = x;
  }
  const std::map<std::string, ir::value*>& globals() const { return globals_; }
  // Metadata
  void add_metadata(const std::string& name, md_pair_t x) {
    metadatas_[name] = x;
  }
...
#endif  // TENSORSCRIPT_TRITON_IR_MODULE_H