#include "tensorscript/codegen/transform/peephole.h"
...
ir::value* rewrite_trans_phi_impl(ir::value* value, ir::builder& builder,
  ...
  if (auto phi = dynamic_cast<ir::phi_node*>(value)) {
    ...
    for (unsigned n = 0; n < phi->get_num_incoming(); n++)
      incs.push_back(
          rewrite_trans_phi_impl(phi->get_incoming_value(n), builder, perm));
...
bool peephole::rewrite_dot(ir::instruction* value, ir::builder& builder) {
  ...
  if (add && add->get_op() == ir::binary_op_t::FAdd) {
    ...
    ir::value* new_dot =
        builder.insert(ir::dot_inst::create_nn(a, b, other, dot->get_name()));
...
bool peephole::rewrite_unit_red(ir::instruction* value, ir::builder& builder) {
  ...
  if (shapes[x->get_axis()] == 1) {
    ...
    ir::value* new_red =
        builder.create_reshape(arg, x->get_type()->get_tile_shapes());
...
bool peephole::rewrite_gep_ptr_min_off_plus_off(ir::instruction* value,
                                                ir::builder& builder) {
