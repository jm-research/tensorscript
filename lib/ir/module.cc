...
/* Module */
module::module(const std::string& name, context& ctx) :name_(name),
    context_(ctx), builder_(ctx) {
  sealed_blocks_.insert(nullptr);
}

ir::builder& module::get_builder() { return builder_; }

ir::context& module::get_context() { return context_; }

void module::set_value(const std::string& name, ir::basic_block* block,
                       ir::value* value) {
...
void module::set_const(const std::string& name) { const_.insert(name); }

void module::set_continue_fn(std::function<ir::value*()> fn) {
  continue_fn_ = fn;
}

std::function<ir::value*()> module::get_continue_fn() { return continue_fn_; }

ir::phi_node* module::make_phi(ir::type* ty, unsigned num_values,
...
ir::value* module::try_remove_trivial_phis(ir::phi_node*& phi) {
  ...
  std::copy_if(phi->ops().begin(), phi->ops().end(),
               std::inserter(non_self_ref, non_self_ref.begin()),
...
ir::value* module::add_phi_operands(const std::string& name,
                                    ir::phi_node*& phi) {
  // already initialized
...
ir::value* module::get_value_recursive(const std::string& name,
                                       ir::basic_block* block) {
  ir::value* result;
  ...
  if (block && !is_const &&
      sealed_blocks_.find(block) == sealed_blocks_.end()) {
...
const std::string& module::get_name() { return name_; }
...
/* functions */
function* module::get_or_insert_function(const std::string& name,
                                         function_type* ty) {
