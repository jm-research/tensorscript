...
const size_t type::get_tile_rank() const { return get_tile_shapes().size(); }
...
bool type::is_int_or_tileint_ty() { return get_scalar_ty()->is_integer_ty(); }
...
bool type::is_sized() const {
  ...
  if (is_integer_ty() || is_floating_point_ty() || is_pointer_ty()) {
    return true;
  }
...
integer_type* type::get_int128_ty(context& ctx) {
  return &ctx.p_impl->int128_ty;
}
...
bool pointer_type::is_valid_elt_ty(type* ty) {
  return !ty->is_void_ty() && !ty->is_label_ty() && !ty->is_metadata_ty() &&
         !ty->is_token_ty();
}
...
bool tile_type::is_valid_elt_ty(type* ty) {
  return ty->is_pointer_ty() || ty->is_floating_point_ty() ||
         ty->is_integer_ty();
}
...
unsigned tile_type::get_bitwidth() const {
  return get_num_elements() *
         get_tile_element_ty()->get_primitive_size_in_bits();
}
...
function_type* function_type::get(type* ret_ty,
                                  const std::vector<type*>& param_tys) {
